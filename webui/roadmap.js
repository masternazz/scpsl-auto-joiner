  ...
  function addRoadmapTools() {
    ...
    if (document.querySelector('.settings-page') && !document.querySelector('#roadmapSettings')) {
      ...
      panel.innerHTML = '<div class="section-heading"><h2>Background services</h2><p>Optional tools run locally and stay disabled until you enable them.</p></div>' +
        ...
        '<div class="setting-row" id="discordPresence"><div><strong>Discord Rich Presence</strong><small>Discord is optional. Server names and player counts are shared only when explicitly enabled.</small></div><label class="toggle-row"><span>Enable</span><input type="checkbox" id="toggleDiscord"></label></div>' +
        '<div class="setting-row" id="discordApplication"><div><strong>Discord application</strong><small>Create a Discord application, paste its public Application ID, then upload the purple S mark with asset key <code>scpsl-autojoin-s</code>.</small><div class="actions"><input class="input" id="discordApplicationId" inputmode="numeric" placeholder="Discord Application ID"><button class="button" id="saveDiscordApplicationId">Save ID</button></div><small id="discordApplicationStatus" role="status"></small></div></div>' +
        '<div class="setting-row" id="discordPlayerSharing"><div><strong>Share player counts</strong><small>Requires Discord and the selected server to allow presence sharing.</small></div><label class="toggle-row"><span>Share</span><input type="checkbox" id="toggleDiscordPlayers"></label></div>';
...
(function () {
  function installServerProfileEditor() {
    ...
    panel.innerHTML = '<div class="section-heading"><h2>Server profile</h2><p>Optional overrides apply only when this destination is joined or monitored.</p></div>' +
      ...
      '<label class="toggle-row"><span><strong>Share player count to Discord</strong><small>Requires global Discord sharing to be enabled.</small></span><input type="checkbox" id="profilePresence"></label>' +
